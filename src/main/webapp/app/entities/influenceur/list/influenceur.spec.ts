...
import { sampleWithRequiredData } from '../influenceur.test-samples';
...
describe('Influenceur Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Influenceur;
  let fixture: ComponentFixture<Influenceur>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Influenceur);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 16800 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.influenceurs()[0]).toEqual(expect.objectContaining({ id: 16800 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 16800 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.influenceurs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 16800 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to influenceurService', () => {
      ...
      vitest.spyOn(service, 'getInfluenceurIdentifier');
      const id = comp.trackId(entity);
      expect(service.getInfluenceurIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.influenceursParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.influenceursParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
