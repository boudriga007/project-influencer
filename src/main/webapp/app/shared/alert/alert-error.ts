...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('tnInfluenceApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertErrorModel>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = this.eventManager.subscribe('tnInfluenceApp.httpError', (response: EventWithContent<unknown> | string) => {
